...
async def _handle_deepen(job: asyncpg.Record) -> None:
    """Deepen an in-graph paper that came up in conversation (Theme A2)."""
    from .pipeline.deepen import deepen
    res = await deepen(job["root_paper_id"], job_id=job["id"])
    log.info("deepen job %s: %s", job["id"], res)
...
HANDLERS: dict[str, Handler] = {
    ...
    "deepen": _handle_deepen,             # ambient deepen-on-mention (Theme A2)
